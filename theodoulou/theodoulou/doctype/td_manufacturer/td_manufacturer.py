...
class TDManufacturer(Document):
	...
	def load_from_db(self):
		...
		data = frappe.db.sql(f"""
			...
			FROM `110` AS `tabTD Manufacturer`
				JOIN `100` AS T100 ON T100.HERNR = `tabTD Manufacturer`.HERNR
		...
		data = frappe.db.sql(f"""
			...
			WHERE 1
				AND (CASE
						...
						WHEN `tabTD Manufacturer`.BJVON <= CAST(CONCAT(@NEEDYEAR,'01') AS UNSIGNED) AND IFNULL(`tabTD Manufacturer`.BJBIS, CAST(CONCAT(YEAR(NOW()),'12') AS UNSIGNED)) >= CAST(CONCAT(@NEEDYEAR,'01') AS UNSIGNED) THEN 1
						ELSE 0
					END) = 1
			ORDER BY {order_by}
        	LIMIT %s, %s;
