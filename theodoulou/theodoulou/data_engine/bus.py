...
class BusQuery(TheodoulouQuery):
    ...
    def get_brands(self):
        data = frappe.cache().get_value('bus_brands')
        if data is None:
            data = frappe.db.sql(f"""
                ...
                ORDER BY NAME;
            """, as_dict=True)
            frappe.cache().set_value('bus_brands', data)

        return data
    
    def get_models(self, ManNo, NeedYear = 0):
            
            data = frappe.cache().get_value('bus_models' + '_' + ManNo + '_' + NeedYear)
            ...
            if data is None:
                ...
                """, as_dict=True)
                frappe.cache().set_value('bus_models' + '_' + ManNo + '_' + NeedYear, data)
